...
export default function CardItem({
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center bg-gray-50 rounded-lg p-3 h-36 md:h-44 lg:h-48 overflow-hidden">
      ...
      <div className="mt-3 flex items-center justify-between gap-3">
        {product.save > 0 && (
          <div className="text-sm text-green-600 font-semibold">
            Save - {formatCurrency(product.save)}
          </div>
        )}
        <div className="flex items-center gap-2">
           {cartCounts[product.id] > 0 ? (
             <QuantitySelector
                ...
                isMaxReached={cartCounts[product.id] >= product.stock}
                onIncrease={() => updateQuantity(product.id, (cartCounts[product.id] || 0) + 1, product.stock)}
                onDecrease={() => updateQuantity(product.id, (cartCounts[product.id] || 0) - 1, product.stock)}
             />
           ) : (
            <AddToCartButton
